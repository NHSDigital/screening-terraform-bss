...
echo "starting scan"
start_scan="$(aws ecr start-image-scan --registry-id "${REGISTRY_ID}" --repository-name "${REPO_NAME}" --image-id imageTag="${IMAGE_TAG}" --region ${AWS_REGION})"
echo "waiting for scan to finish"
$(aws ecr wait image-scan-complete --registry-id ${REGISTRY_ID} --repository-name ${REPO_NAME} --image-id imageTag=${IMAGE_TAG} --region ${AWS_REGION})
echo "getting  results"
$(aws ecr describe-image-scan-findings --registry-id ${REGISTRY_ID} --repository-name ${REPO_NAME} --image-id imageTag=${IMAGE_TAG} --region ${AWS_REGION} >scan_results.json)
